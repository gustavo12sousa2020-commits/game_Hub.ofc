...
app.post("/api/auth/cadastro", async (req, res) => {
    ...
    if (nome.length < 2 || nome.length > 40)
        return res.status(400).json({ erro:"Nome deve ter 2–40 caracteres." });
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
    ...
    const novoUser = {
        ...
        senha: hash,
        token,
        avatar: nome.trim()[0].toUpperCase(),
...
app.get("/api/admin/usuarios", mwAdmin, (req, res) => {
    const lista = lerJSON(USUARIOS_DB,[]).map(u => ({
        id: u.id, nome: u.nome, email: u.email,
        dataCadastro: u.dataCadastro, role: u.role
    }));
...
app.delete("/api/admin/usuarios", mwAdmin, (req, res) => {
    const { id } = req.body;
    const lista = lerJSON(USUARIOS_DB,[]).filter(u => u.id !== id);
    salvarJSON(USUARIOS_DB, lista);
    res.json({ ok:true });
});
...
async function buscarNewsAPI() {
    const res = await axios.get("https://newsapi.org/v2/everything", {
        params: {
            q: '(gaming OR videogames OR esports OR anime OR manga OR "one piece" OR "jujutsu kaisen" OR "demon slayer" OR "dragon ball" OR "attack on titan" OR "video game" OR nintendo OR playstation OR xbox OR steam)',
            language: "pt", sortBy: "publishedAt", pageSize: 100, apiKey: API_KEY
        }
    });
...
async function buscarGNews() {
    ...
    try {
        const res = await axios.get("https://gnews.io/api/v4/search", {
            params: { q:"games OR anime OR esports", lang:"pt", max:20, apikey: GNEWS_KEY }
        });
        return (res.data.articles || []).map(a => ({
            title: a.title, description: a.description, url: a.url,
            urlToImage: a.image, publishedAt: a.publishedAt,
            source: { name: a.source?.name || "GNews" }, author: null, content: a.content
        }));
    } catch { return []; }
}
...
app.get("/api/noticias", async (req, res) => {
    try {
        ...
        if (!artigos.length) {
            const db = lerJSON(NOTICIAS_DB, { cache:[] });
            artigos = db.cache || [];
        }
        res.json(artigos);
    } catch {
        const db = lerJSON(NOTICIAS_DB, { cache:[] });
        if (db.cache?.length) return res.json(db.cache);
...
app.get("/api/noticias-topico", async (req, res) => {
    ...
    try {
        const r = await axios.get("https://newsapi.org/v2/everything", {
            params: { q, language:"pt", sortBy:"publishedAt", pageSize:20, apiKey:API_KEY }
        });
...
app.get("/api/noticia", async (req, res) => {
    ...
    try {
        ...
        let c = $("article");
        if (!c.length) c = $('[itemprop="articleBody"]');
        if (!c.length) c = $(".post-content,.entry-content,.article-content,.article-body");
        if (!c.length) c = $("main");
        if (!c.length) c = $("body");
        const ps = [];
        c.find("p").each((i,el) => { const t=$(el).text().replace(/\s+/g," ").trim(); if(t.length>40) ps.push(t); });
...
app.get("/api/comentarios", (req, res) => {
    ...
    const u = validarUserToken(req);
    const lista = lerJSON(COMENTARIOS_DB,[]).filter(c=>c.pagina===p).slice(-50).reverse();
    res.json(lista);
});
...
app.post("/api/comentarios", (req, res) => {
    ...
    todos.push({
        id: Date.now(), pagina: pagina||"geral",
        nome: nomeReal, texto: texto.trim(),
...
app.delete("/api/admin/comentarios", mwAdmin, (req,res) => {
    const { id } = req.body;
    salvarJSON(COMENTARIOS_DB, lerJSON(COMENTARIOS_DB,[]).filter(c=>c.id!==id));
    log("remove_comentario", String(id), req.ip);
    res.json({ ok:true });
});
...
app.delete("/api/admin/doadores", mwAdmin, (req,res) => {
    const { id } = req.body;
    salvarJSON(DOADORES_DB, lerJSON(DOADORES_DB,[]).filter(d=>d.id!==id));
    res.json({ ok:true });
});
...
app.get("/api/admin/stats", mwAdmin, (req,res) => {
    const noticias  = lerJSON(NOTICIAS_DB,{cache:[]}).cache?.length || 0;
    const comentarios = lerJSON(COMENTARIOS_DB,[]).length;
    const usuarios  = lerJSON(USUARIOS_DB,[]).length;
    const doadores  = lerJSON(DOADORES_DB,[]).length;
    const totalPix  = lerJSON(DOADORES_DB,[]).reduce((s,d)=>s+d.valor,0);
    res.json({ noticias, comentarios, usuarios, doadores, totalPix });
});
